...
本项目灵感来自旅行者1号（Voyager 1）携带的镀金唱片（Voyager Golden Record）——
...
本次更新聚焦在 **性能提升、内存优化、日志清理、可观测性增强**，不改变项目的基本使用方式。

### 1) 编解码支持多核并行

* 编码阶段支持多进程并行生成音频分块。
* 解码阶段支持多进程并行 FFT 反推灰度值。
* 命令行新增可选参数 `workers`（并行进程数）：

```bash
python app.py encode input.jpg signal.flac [workers]
python app.py draw signal.flac recovered.png [workers]
```

> 不传 `workers` 时会自动使用 CPU 核心数。

### 2) 内存占用大幅优化（流式处理）

* 编码改为分块生成 + 分块写入 FLAC，避免整段音频常驻内存。
* 解码改为分块读取音频 + 分块计算 FFT，避免一次性加载大数组。
* 去除全量像素坐标缓存，改为索引实时映射坐标。
* 播放改为 `pygame.mixer.music` 流式播放，降低峰值内存。

### 3) 运行输出更干净

* pygame 改为按需导入，仅在解码显示时加载。
* 编码多进程场景下，不再重复刷屏输出 pygame 欢迎信息。
* 已对相关弃用警告做了定向抑制，终端日志更聚焦核心信息。
...
* `ENCODE_COLUMN_BLOCK`：编码每个任务处理的列数
* `DECODE_PIXEL_BLOCK`：解码每批处理的像素数
